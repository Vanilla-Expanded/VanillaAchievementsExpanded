    ...
    internal static class AchievementHarmony
    {
        ...
        public static void PawnJoinedFaction(Pawn p, PopAdaptationEvent ev)
        {
            foreach (var card in AchievementPointManager.GetCards<PawnJoinedTracker>())
            {
                ...
                {
                    LogCardValidationError(card, ex);
                }
            }
        }
        ...
        public static void SingleLongTickTracker()
        {
            ...
            if (ticks % 2000 == 0)
            {
                ...
                AchievementPointManager.tickerAchievements.RemoveWhere(c => c.unlocked);
            }
            if (ticks % 60000 == 0)
            {
                foreach (var card in AchievementPointManager.GetDailyTickCards())
                {
                    try
                    {
                        if (card.tracker.AttachToDailyTick())
                        {
                            card.UnlockCard();
                        }
                    }
                ...
                AchievementPointManager.dailyTickerAchievements.RemoveWhere(c => c.unlocked);
            }
        }
        /// <summary>
        ...
        public static void GravshipLanded(Gravship ___gravship)
        {
            if (Current.ProgramState == ProgramState.Playing)
            {
                foreach (var card in AchievementPointManager.GetCards<GravshipLandingTracker>())
                {
                    try
                    {
                        if ((card.tracker as GravshipLandingTracker).Trigger(___gravship))
                        {
                            card.UnlockCard();
                        }
                    }
        ...
        /// <summary>
        /// Gravship takeoff Event
        /// </summary>

        public static void GravshipTakeoff(Building_GravEngine engine, PlanetTile targetTile)
        {
            if (Current.ProgramState == ProgramState.Playing)
            {
                foreach (var card in AchievementPointManager.GetCards<GravshipLaunchContentsTracker>())
                {
                    try
                    {
                        if ((card.tracker as GravshipLaunchContentsTracker).Trigger(engine))
                        {
                            card.UnlockCard();
                        }
                    }
        ...
        public static void MapEntered(Map __result)
        {
            if (Current.ProgramState == ProgramState.Playing)
            {
                foreach (var card in AchievementPointManager.GetCards<MapEnteringTracker>())
                {
                    ...
                    {
                        LogCardValidationError(card, ex, __result);
                    }
                }
            }
        }
        ...
        public static void AbandonMap(bool wasGravshipLaunch)
        {
            if (Current.ProgramState == ProgramState.Playing)
            {
                foreach (var card in AchievementPointManager.GetCards<AbandonedMapTracker>())
                {
                    try
                    {
                        if ((card.tracker as AbandonedMapTracker).Trigger(wasGravshipLaunch))
                        {
                            card.UnlockCard();
                        }
                    }
        ...
        public static void LogCardValidationError(AchievementCard card, Exception ex, Map passingMap = null)
        {
            ...
            card.UnlockCard(passingMap: passingMap);
        }
    }
}
